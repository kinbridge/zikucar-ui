<template>
  <div>
          ...
          <el-row :gutter="20">
            <el-col :span="24" style="padding: 15px; ">
              <el-row :gutter="20" style="background: #f4f4f5;padding:10px;">
                <el-col :span="5">
                  <el-input placeholder="岗位名称" v-model="pageQuery.sysPosition.positionName"
    ...
    <el-row>
      <el-button @click="addposition" type="primary">新增</el-button>
    ...
    <el-table :data="tableData" ref="table" style="width: 100%">
      ...
      <el-table-column class="text-overflow"
                       :show-overflow-tooltip="true" prop="deptId" label="部门" :formatter="deptFormatter">
      </el-table-column>
      <el-table-column class="text-overflow"
                       :show-overflow-tooltip="true" prop="positionName" label="岗位名称">
      </el-table-column>
      <el-table-column class="text-overflow"
                       :show-overflow-tooltip="true" prop="positionCode" label="岗位编号">
      </el-table-column>
      <el-table-column class="text-overflow"
                       :show-overflow-tooltip="true" prop="remark" label="岗位描述">
      </el-table-column>
    </el-table>
    <el-pagination background layout="prev, pager, next" ></el-pagination>

    <!--    编辑框-->
    <position-edit :dialogVisible="dialogVisible" v-if="dialogVisible" :params="params"
               @position-edit-close="positionEditClose"></position-edit>
    <!--    分页组件-->
  ...
  methods: {
    ...
    handleDelete(index, row) {
      this.$confirm('确认删除岗位吗?', '提示', {
        ...
        this.http.get(this.api.position.deleteById + "/" + row.pkId, res => {
          if (res.code == 200) {
    ...
    getData() {
      this.http.post(this.api.position.pageList, this.pageQuery,res => {
